...
FileCopy(@ScriptDir & "\Publish\RAMM_" & $MM_VERSION & ".exe", $sPath & "\RAMM\" & $MM_VERSION_SUBTYPE & "\RAMM_latest.exe", $FC_OVERWRITE + $FC_CREATEPATH)
FileCopy(@ScriptDir & "\Publish\RAMM_" & $MM_VERSION & ".zip", $sPath & "\RAMM\" & $MM_VERSION_SUBTYPE & "\RAMM_latest.zip", $FC_OVERWRITE + $FC_CREATEPATH)
